...
bool VoxbloxMap::getDistanceAndGradientAtPosition(const Eigen::Vector3d& position,
                                        double* distance,
                                        Eigen::Vector3d* gradient)
{
          std::cout <<" using  Voxblox  for Gradient calculation "  <<std::endl;

         if (esdf_server_->getEsdfMapPtr()->getDistanceAndGradientAtPosition(position, distance, gradient))
            {
            return true;
            }
              else 
...
int VoxbloxMap:: getMapSize()
{
    int b= esdf_server_->getEsdfMapPtr()->getEsdfLayer().getNumberOfAllocatedBlocks();
    return b;
    // std::cout <<"getMapSize   vps===="<<b<<std::endl;
}
