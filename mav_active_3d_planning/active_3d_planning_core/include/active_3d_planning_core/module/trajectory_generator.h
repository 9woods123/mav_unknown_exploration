...
class TrajectoryGenerator : public Module {
 public:
  ...
  virtual bool SetRRTStepLength(int steps_length, const TrajectorySegment& segment, int replan_signal);
  
  // virtual bool SetVoxbloxMap(voxblox::EsdfServer& voxbloxESDFserver) ;
  ...
  //woods            get the map ptr for gradient .
  Map& getMap();
