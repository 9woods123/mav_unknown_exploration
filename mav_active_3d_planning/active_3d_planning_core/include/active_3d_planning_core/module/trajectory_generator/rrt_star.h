...
// std::unique_ptr<TrajectorySegment>
  bool  selectNextBestMultiStep(TrajectorySegment*  traj_in, std::vector<int> & children_index) override;

  bool setMultiStepsLength(int length) override;
  ...
  bool  woodsMultiStep( TrajectorySegment*  traj_in,int* children_index)override;
 ...
 protected:
  ...
  int multi_step_number;  // Force reset of kdtree if requestNext is called twice
