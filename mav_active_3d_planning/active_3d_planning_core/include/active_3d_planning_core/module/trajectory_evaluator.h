...
class TrajectoryEvaluator : public Module {
 public:
  ...
  virtual bool selectNextBestMultiStep(TrajectorySegment*  traj_in,
 std::vector<int> & children_index);

   virtual bool setMultiStepsLength(int length);

  virtual  bool  woodsMultiStep( TrajectorySegment*  traj_in,int* children_index);
 ...
 protected:
  ...
  std::string next_setps_selector_args_;
  // default modules
  ...
  std::unique_ptr<NextStepsSelector> next_steps_selector_;
...
class NextStepsSelector : public Module {
 public:
  explicit NextStepsSelector(PlannerI& planner) : Module(planner) {}

  virtual bool selectNextBestMultiStep(TrajectorySegment*  traj_in,
 std::vector<int> & children_index)=0;
 };
