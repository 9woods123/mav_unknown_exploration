...
class OnlinePlanner : public PlannerI, public ModuleBase {
 ...
 protected:
  ...
  std::clock_t
      woods_time_init;         // Simulated/On-Board time for verbose and perf [s]
  ...
  std::clock_t perf_cpu_timer_;  // total time counter
   float   traj_cost;
   float   gain_evaluation_time;

   std::ofstream woods_data;   //////   =====woods======   record test data
  ...
  // conditions
  bool multi_or_singe_step_; // true: execute multi step, false:singe step.
  int multi_step_length_;
  ...
  virtual void requestMovement(
      const EigenTrajectoryPoint& trajectory_point) = 0;
  ...
  virtual void publishwoodsTrajectoryVisualization(
