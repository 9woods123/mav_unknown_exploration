...
void TrajectoryEvaluator::setupFromParamMap(Module::ParamMap* param_map) {
  ...
  std::cout<<"==============="<<p_next_args_<<"==============="<<std::endl;

  // setParam<std::string>(param_map, "next_setps_selector_args", &next_setps_selector_args_,
  //                       ns + "/next_setps_selector");                      
...
bool TrajectoryEvaluator::setMultiStepsLength(int length)
{
  return true;
}

bool TrajectoryEvaluator::selectNextBestMultiStep(TrajectorySegment*  traj_in,
 std::vector<int> & children_index)
{
  // If not implemented use a (default) module
  if (!next_steps_selector_) {
    next_steps_selector_ = planner_.getFactory().createModule<NextStepsSelector>(
     p_next_args_ , planner_, verbose_modules_);

  }
  next_steps_selector_ ->selectNextBestMultiStep( traj_in, children_index);
  return true;
}

  bool TrajectoryEvaluator::woodsMultiStep( TrajectorySegment*  traj_in,int* children_index)
  {
       std::cout <<"======TrajectoryEvaluator::woodsMultiStep========="<<std::endl;
      return true;
  }
