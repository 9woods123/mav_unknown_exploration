...
//  generator_->rewireRoot(traj_in, &children_index[0]);   current_segment_.get(),next_segments
...
bool RRTStar::rewireToBestParent(
  ...
  void RRTStar::Set_reset_be_true()
  {
  tree_is_reset_=true;    
  }
...
bool RRTStarEvaluatorAdapter::selectNextBestMultiStep(TrajectorySegment*  traj_in, std::vector<int> & children_index)
 {
  following_evaluator_->selectNextBestMultiStep(traj_in,children_index);
  return true;
}

bool RRTStarEvaluatorAdapter::setMultiStepsLength(int length) 
{
multi_step_number=length;
}

  bool RRTStarEvaluatorAdapter::woodsMultiStep(TrajectorySegment* traj_in, int* children_index)
  { 

      generator_->rewireRoot(traj_in, children_index);
      // std::cout<<"====in====children_index="<<*children_index<<std::endl;
      multi_step_number--;
      if(multi_step_number>0)
      {
      generator_->Set_reset_be_true();
      }
      return true;
  }
...
void RRTStarEvaluatorAdapter::setupFromParamMap(Module::ParamMap* param_map) {
  ...
  std::cout<<"RRTStarEvaluatorAdapter==args==="<<args <<std::endl; 
   std::cout<<"RRTStarEvaluatorAdapter==param_ns==="<<param_ns <<std::endl; 
  std::cout<<"==RRTStarEvaluatorAdapter===++following_evaluator" <<std::endl;               
