...
bool RRT::sampleGoal(Eigen::Vector3d* goal_pos) {
  if (p_sampling_mode_ == "uniform") {
    ...
    if (semilocal_count_ > 0) {
      // Not enough local points foun d, sample from local sphere
      //===========woods===========  adjust the theta here to get smooth path locally
      double theta = 2.0 * M_PI * static_cast<double>(rand()) / RAND_MAX;                    //Original
      // double theta = 0.5 * M_PI *(1-2* static_cast<double>(rand()) / RAND_MAX);                   //WOODS
...
bool RRT::sampleGoal(Eigen::Vector3d* goal_pos, TrajectorySegment* root){
  ...
  if (p_sampling_mode_ == "uniform") {
    ...
    if (semilocal_count_ > 0) {
      // Not enough local points foun d, sample from local sphere
      /////   local kinodynamic sampling
      double  v_max=1.5;  
      double  u_xy_max=1.5;  double u_z_max=0.5;          //  u =   d^2 x/d t^ 2     which means  that we input the  acceleration  as the control
      double u_yaw_max=3;
      double dt=1;
      double random_control=(static_cast<double>(rand()) / RAND_MAX) *2 -1;           // r  belong to -1~1
      double  control_acc=random_control*u_xy_max;
      double  a_x=(static_cast<double>(rand()) / RAND_MAX) * control_acc*v_max;
      double  a_y=control_acc*v_max  - a_x;
      int traj_size= root->trajectory.size();
      Eigen::Vector3d   current_goal_velocity(0,0,0);
      double current_goal_vel_max=sqrt(
        pow(root->trajectory.at(traj_size-1).velocity_W.x(),2)+
        pow(root->trajectory.at(traj_size-1).velocity_W.y(),2)+
        pow(root->trajectory.at(traj_size-1).velocity_W.z(),2)
      );
      if (traj_size>=2)
      {
        current_goal_velocity[0]= v_max* root->trajectory.at(traj_size-1).velocity_W.x()/current_goal_vel_max;
        current_goal_velocity[1]= v_max*root->trajectory.at(traj_size-1).velocity_W.y()/current_goal_vel_max;
        current_goal_velocity[2]= v_max*root->trajectory.at(traj_size-1).velocity_W.z()/current_goal_vel_max;
      }
      // std::cout <<" velocity_x=="<< current_goal_velocity[0]<<std::endl;
      // std::cout <<" velocity_y=="<< current_goal_velocity[1]<<std::endl;

      *goal_pos+=Eigen::Vector3d(
        current_goal_velocity[0]*dt+0.5*dt*dt*a_x,                                  ///   a_y= a *v - a_x
        current_goal_velocity[1]*dt+0.5*dt*dt*a_y,
        current_goal_velocity[2]*dt+0.5*dt*dt*random_control*u_z_max
        );
      
        // v_max*dt+0.5*dt*dt*random_control*u_yaw_max
...
bool RRT::adjustGoalPosition(const Eigen::Vector3d& start_pos,
  ...
  // std::cout<<"======adjustGoalPosition======"<<std::endl;
      ...
      if (!checkTraversable(start_pos + static_cast<double>(i) /
                                            ...
                                            direction)) 
                                            {
                                                                double length = direction.norm() * static_cast<double>(i) /
                                                                                    static_cast<double>(n_points) - p_crop_margin_;
