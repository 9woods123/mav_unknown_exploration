...
RosPlanner::RosPlanner(const ::ros::NodeHandle& nh,
  ...
  traj_pub_= nh_.advertise<trajectory_msgs::MultiDOFJointTrajectory>(
      "traj_server/trajectory", 1);
  trajectory_vis_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "trajectory_visualization", 100);
  local_trajectory_vis_pub_= nh_.advertise<visualization_msgs::MarkerArray>(
      "local_trajectory_visualization", 100);
  odom_sub_ = nh_.subscribe("odometry", 1, &RosPlanner::odomCallback, this);
  replan_singal_sub_=nh_.subscribe("replan_singal",1, &RosPlanner::replan_singal_Callback, this);
...
void RosPlanner::planningLoop() {
  ...
  //   woodsexptime+=( ::ros::Time::now() - computationTime).toSec();
  //   std::cout<<"Total_Flying_Time=="<<woodsexptime<<" s"<<std::endl;
  }
}
...
bool RosPlanner::requestNextTrajectory() {
  ...
  // double woodsexptime=( ::ros::Time::now() - computationTime).toSec();
  // std::cout<<"==========computationTime = "<<woodsexptime<<"==========="<<std::endl;
  // Performance log
  if (p_log_performance_) {
...
void RosPlanner::replan_singal_Callback(const std_msgs::Int32& msg)
{
  if(msg.data>0)
  {
      replan_signal_=msg.data;
  }
}
...
void RosPlanner::requestMovement(const EigenTrajectoryPoint& trajectory_point) {
  ...
  for (int i = 0; i < n_points; ++i) {
    msgMultiDofJointTrajectoryPointFromEigen(trajectory_point, &msg->points[i]);
  }
...
void RosPlanner::publishVisualization_local_traj(const VisualizationMarkers& markers) {
  if (markers.getMarkers().empty()) {
    return;
  }
  visualization_msgs::MarkerArray msg;
  ...
  local_trajectory_vis_pub_.publish(msg);
}
