...
bool LocoSmoother::getTrajectoryBetweenWaypoints(
  ...
  // }
    // mav_trajectory_generation::sampleWholeTrajectory(*trajectory, 0.1, &path);
  ...
  // loco.setRobotRadius(constraints_.robot_radius);
    loco.setRobotRadius(0.5);
    loco.setMapResolution(0.1);


  if (distance_and_gradient_function_) {
        // std::cout <<"============distance_and_gradient_function_ ============"<<std::endl;

    loco.setDistanceAndGradientFunction(
        std::bind(&LocoSmoother::getMapDistanceAndGradient, this,
                  std::placeholders::_1, std::placeholders::_2));
  } else {
    // std::cout <<"============ map_distance_func_ ============"<<std::endl;
...
if(position.size()==4)
{
  position_3d=Eigen::Vector3d(position[0],position[1],position[2]);
}
  ...
  if (gradient == nullptr) {
    ...
    return distance_and_gradient_function_(position_3d, nullptr);

  }
  Eigen::Vector3d gradient_3d;

  // double distance = distance_and_gradient_function_(position, &gradient_3d);
  double distance = distance_and_gradient_function_(position_3d, &gradient_3d);

if(position.size()==4)
{
  *gradient =Eigen::Vector4d(gradient_3d[0],gradient_3d[1],gradient_3d[2],0);
}else{
